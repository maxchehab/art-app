...
export default class GameBard extends Component {
     ...
     constructor(props) {
          ...
          this.state = {
               ...
               modalSource: "https://static.independent.co.uk/s3fs-public/styles/article_small/public/thumbnails/image/2016/03/10/18/mona-lisa.jpg"
          }
          this.generateImages();
     }
          ...
          if (id != null) {
               ...
               for (var i = 0; i < this.state.imagesData.length; i++) {
                    if (this.state.imagesData[i].id == id) {
                         this.setState({modalTitle: this.state.imagesData[i].name, modalDescription: this.state.imagesData[i].description, modalSource: this.state.imagesData[i].source})
                         break;
                    }
               }
          }
     ...
     generateImages() {
          ...
          const download = async() => {
               ...
               let responseJson = await response.json();
               await AsyncStorage.setItem("version", JSON.stringify(responseJson.version));
          ...
          const generate = async() => {
               get().then(function(images) {
                    ...
                    for (var i = 0; i < images.length; i++) {
                         Images.push(<Card key={i} compare={that.press.bind(that)} clickable={true} index={images[i].id} onFlipEnd={that.compare.bind(that)} source={images[i].source}/>)
                    }
          ...
          return (
               <ScrollView>
                    ...
                    <Modal transparent={true} animationType="slide" visible={this.state.modalVisible} onRequestClose={() => {
                         ...
                         <TouchableOpacity style={styles.centerModal} activeOpacity={1} onPress={() => {
                              this.setModalVisible(false);
                         }}>
                              <ScrollView directionalLockEnabled={true} contentContainerStyle={styles.centerModal}>
                                   <TouchableWithoutFeedback>
                                        <View style={{
                                             ...
                                             <View style={{
                                                  ...
                                                  <Text style={{
                                                       fontSize: 15
                                                  }}>{this.state.modalDescription}</Text>
                                             </View>
                                        </View>
                                   </TouchableWithoutFeedback>
                              </ScrollView>
